...
export default function AdminUsers({ data, setData, addNotification, currentUser, activeScreen, showAlert }: AdminUsersProps) {
  ...
  const handleOpenEditUser = (u: User) => {
    setSelectedUser(u);
    setUserForm({ name: u.name, phone: u.phone, role: u.role, password_hash: u.password_hash, status: u.status });
  ...
  const handleSaveUser = () => {
    ...
    if (isNew) {
      const newUser: User = { id: 'u_' + Date.now(), ...userForm };
      updatedUsers = [...updatedUsers, newUser];
      let temp = { ...data, users: updatedUsers };
      temp = logAudit(temp, 'USER_CREATE', 'User', newUser.id, currentUser?.id || 'admin', `Registered representative: ${newUser.name} with role ${newUser.role}`);
      setData(temp);
      addNotification('user_update', `New operator account created: ${newUser.name} (${newUser.role}).`);
      showAlert(`User ${newUser.name} registered successfully! Password: ${newUser.password_hash}`);
    } else {
      updatedUsers = data.users.map(u => (u.id === selectedUser?.id ? { ...u, ...userForm } : u));
      let temp = { ...data, users: updatedUsers };
      temp = logAudit(temp, 'USER_EDIT', 'User', selectedUser!.id, currentUser?.id || 'admin', `Modified details of operator: ${userForm.name}`);
      setData(temp);
  ...
  const handleToggleUserStatus = (u: User) => {
    ...
    const updatedUsers = data.users.map(user => (user.id === u.id ? { ...user, status: nextStatus as any } : user));
    let temp = { ...data, users: updatedUsers };
    temp = logAudit(temp, 'USER_TOGGLE', 'User', u.id, currentUser?.id || 'admin', `Toggled status of operator ${u.name} to ${nextStatus}`);
    setData(temp);
    addNotification('user_update', `${u.name}'s account was ${nextStatus === 'Active' ? 'reactivated' : 'disabled'}.`);
    showAlert(`Representative ${u.name} status updated to: ${nextStatus}`);
  };
  ...
  const handleConfirmResetPassword = () => {
    ...
    const updatedUsers = data.users.map(user => (user.id === resetPasswordUserId ? { ...user, password_hash: resetPasswordValue } : user));
    let temp = { ...data, users: updatedUsers };
    temp = logAudit(temp, 'USER_RESET_PW', 'User', resetPasswordUserId, currentUser?.id || 'admin', `Reset password credentials for operator UID ${resetPasswordUserId}`);
    setData(temp);
        ...
        activeForm === 'add_user' || activeForm === 'edit_user' ? (
          <View className="bg-white p-4 rounded-2xl border border-slate-200 gap-4">
            ...
            <View className="gap-3">
              <View>
                ...
                <TextInput value={userForm.name} onChangeText={v => setUserForm({ ...userForm, name: v })} placeholder="e.g. John Doe" placeholderTextColor="#94a3b8" className={inputClass} />
              </View>
          ...
          <View className="gap-3">
            ...
            <View className="gap-2.5">
              {data.users.map(u => (
                <View key={u.id} className="bg-white rounded-2xl p-3 border border-slate-200 flex-row items-center justify-between">
                  <View className="flex-row items-center gap-2.5 flex-1">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-[10px] text-slate-400 mt-0.5">Role: {u.role} - U: {u.phone} - P: {u.password_hash}</Text>
                    </View>
                  </View>
          ...
          <View className="gap-2">
            {data.notifications.map(n => (
              <Pressable
                ...
                className={`p-3 rounded-2xl border flex-row gap-3 relative ${n.is_read ? 'bg-slate-50 border-slate-200' : 'bg-white border-rose-200'}`}
              >
                {!n.is_read && <View className="absolute top-3 right-3 w-2 h-2 bg-rose-500 rounded-full" />}
                ...
                <View className="flex-1">
                  <View className="flex-row justify-between items-center">
                    <Text className={`font-bold text-[9px] uppercase tracking-wider ${n.is_read ? 'text-slate-500' : 'text-slate-800'}`}>
                      {notifTypeLabels[n.type] || n.type}
                    </Text>
                    <Text className="text-[8px] text-slate-400">{new Date(n.created_at).toLocaleTimeString()}</Text>
                  </View>
