...
export async function loadCargo(truckId: string, items: LoadItem[], actorId: string) {
  ...
  return prisma.$transaction(async tx => {
    for (const item of items) {
      ...
      const available = inv ? (item.source === 'reserved_qty' ? inv.reserved_qty : inv.available_qty) : 0;
      if (item.qty > available) {
        throw ApiError.conflict(`Insufficient ${item.source === 'reserved_qty' ? 'reserved' : 'available'} warehouse stock! Only ${available} units of ${product?.name ?? 'product'} are available in that pool (Requested: ${item.qty}).`);
      }
    }
    ...
    for (const item of items) {
      if (item.source === 'reserved_qty') {
        await tx.warehouseInventory.update({ where: { product_id: item.product_id }, data: { reserved_qty: { decrement: item.qty } } });
      } else {
        await tx.warehouseInventory.update({ where: { product_id: item.product_id }, data: { available_qty: { decrement: item.qty } } });
      }
      ...
      if (existingTruckInv) {
        await tx.truckInventory.update({ where: { truck_id_product_id: { truck_id: truckId, product_id: item.product_id } }, data: { quantity: { increment: item.qty } } });
      } else {
        await tx.truckInventory.create({ data: { truck_id: truckId, product_id: item.product_id, quantity: item.qty } });
      }
      ...
      if (item.pre_booking_id) {
        ...
        if (existingDispatch) {
          await tx.preBookingDispatchedItem.update({ where: { id: existingDispatch.id }, data: { quantity: { increment: item.qty } } });
        } else {
          await tx.preBookingDispatchedItem.create({ data: { pre_booking_id: item.pre_booking_id, product_id: item.product_id, quantity: item.qty } });
        }
      }

      const product = await tx.product.findUnique({ where: { id: item.product_id } });
      summaries.push(`${item.qty} units of ${product?.name ?? 'Product'} (${item.source === 'reserved_qty' ? 'Reserved Pool' : 'Available Pool'})`);
    }
...
export async function returnStock(truckId: string, productId: string, qty: number, actorId: string) {
  return prisma.$transaction(async tx => {
    ...
    if (qty > truckInv.quantity) throw ApiError.badRequest(`Cannot return ${qty} units. Only ${truckInv.quantity} units are currently loaded on this truck.`);

    const remaining = truckInv.quantity - qty;
    if (remaining > 0) {
      await tx.truckInventory.update({ where: { truck_id_product_id: { truck_id: truckId, product_id: productId } }, data: { quantity: remaining } });
    } else {
    ...
    await tx.warehouseInventory.upsert({
      ...
      update: { available_qty: { increment: qty } },
      create: { product_id: productId, available_qty: qty },
    });
    ...
    await logAudit({
      ...
      entity_id: truckId,
      user_id: actorId,
      details: `Truck -> Warehouse Return Load: Recalled ${qty} units of unsold ${product?.name ?? 'Product'} back from Truck ${truck?.vehicle_number ?? 'Truck'}`,
    });
...
export async function returnAllStock(truckId: string, actorId: string) {
  return prisma.$transaction(async tx => {
    ...
    for (const item of cargo) {
      ...
      const product = await tx.product.findUnique({ where: { id: item.product_id } });
      summaries.push(`${item.quantity} units of ${product?.name ?? 'Product'}`);
    ...
    return { returnedUnits: totalReturned };
  });
}

// Mirrors handleTruckToTruckTransfer(): move cargo directly between two
// trucks without passing back through the warehouse.
export async function transferBetweenTrucks(fromTruckId: string, toTruckId: string, items: { product_id: string; qty: number }[], actorId: string) {
  ...
  return prisma.$transaction(async tx => {
    for (const item of items) {
      ...
      if (!sourceInv || sourceInv.quantity < item.qty) {
        ...
        throw ApiError.conflict(`Insufficient stock on source truck! Only ${sourceInv?.quantity ?? 0} units of ${product?.name ?? 'Product'} exist on vehicle ${sourceTruck?.vehicle_number ?? 'Truck A'} (Requested total: ${item.qty}).`);
      }
    }
    ...
    for (const item of items) {
      ...
      const remaining = sourceInv.quantity - item.qty;
      if (remaining > 0) {
        await tx.truckInventory.update({ where: { truck_id_product_id: { truck_id: fromTruckId, product_id: item.product_id } }, data: { quantity: remaining } });
      } else {
      ...
      if (destInv) {
        await tx.truckInventory.update({ where: { truck_id_product_id: { truck_id: toTruckId, product_id: item.product_id } }, data: { quantity: { increment: item.qty } } });
      } else {
        await tx.truckInventory.create({ data: { truck_id: toTruckId, product_id: item.product_id, quantity: item.qty } });
      }
    }
