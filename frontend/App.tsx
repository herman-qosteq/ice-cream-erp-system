...
function AppRouter() {
  const { data, setData, addNotification, syncQueueOffline, currentUser, setCurrentUser, pushAlert, login } = useAppContext();
  ...
  return (
    <View className="flex-1 bg-sky-50 relative">
      {!currentUser ? (
        <RoleSelectScreen login={login} setCurrentUser={handleCustomSetCurrentUser} />
      ...
      {/* IN-APP PUSH ALERT BANNER — full-width strip on mobile, but on
          tablet/desktop web it's capped to a fixed-width toast anchored to
          the top-right instead of stretching across the whole browser
          window. */}
      {pushAlert && (
        <View className="absolute top-2 left-3 right-3 md:left-auto md:right-4 md:w-[380px] bg-white border border-rose-500/30 shadow-2xl rounded-2xl p-3 z-50 flex-row items-start gap-2.5">
